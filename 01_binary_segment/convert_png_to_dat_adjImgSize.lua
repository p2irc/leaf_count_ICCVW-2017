...
local dataPath = '/media/aich/DATA/leaf_seg_par/';
local rgbPath = './rgb';
local fgPath = './fg';

local outDataPath = '/media/aich/DATA/leaf_seg_par_dat/';
local outRgbPath = './rgb';
local outFgPath = './fg';
			...
			for fid = 1, #fileList, 1 do
				pcall( function()
					...
					if (fileList[fid] ~= '.') and (fileList[fid] ~= '..') then
						...
						local gt = image.load(tmpFgPath, 'byte');
						gt = torch.squeeze(gt);
						local im = image.load(tmpRgbPath, 3, 'byte');
						...
						if (gt:size()[1] < MIN_SIZE) or (gt:size()[2] < MIN_SIZE) then
							...
							-- save in the input locations
							image.save(tmpFgPath, gt_new);
							image.save(tmpRgbPath, im_new);
							gt_new = gt_new + 1;
...
torch.save('./directory_mean.dat', dirMean);
