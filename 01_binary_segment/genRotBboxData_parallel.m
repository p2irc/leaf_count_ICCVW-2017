function genRotBboxData_parallel()
restoredefaultpath;
...
rgbPath = 'rgb';
fgPath = 'fg';
outBasePath = '/media/aich/DATA/leaf_seg_par/';
...
%             subplot(2,2,4); imshow(im_fg2);
            ...
            if ~isempty(im_rgb1)
                ...
                imgName = [num2str(i), '_', num2str(rot)];
                imwrite(im_rgb1, fullfile(outRgbPath, num2str(randDir), [num2str(imgName), '.png']));
                imwrite(im_fg1, fullfile(outFgPath, num2str(randDir), [num2str(imgName), '.png']));  
                count = count + 1;
            else 
                fprintf('file = %d, rot = %d is empty\n', i, rot);
            end
            if ~isempty(im_rgb2)
                randDir = randi(numOutDirs); % select a random directory
                % numFilesPerDir(randDir) = numFilesPerDir(randDir) + 1;
                % imgNum = numFilesPerDir(randDir);
                imgName = [num2str(i), '_fliplr_', num2str(rot)];
                imwrite(im_rgb2, fullfile(outRgbPath, num2str(randDir), [num2str(imgName), '.png']));
                imwrite(im_fg2, fullfile(outFgPath, num2str(randDir), [num2str(imgName), '.png']));                         
                count = count + 1;
            else 
                fprintf('file = %d, rot = %d (fliplr) is empty\n', i, rot);                
            end
        end
    end
end
