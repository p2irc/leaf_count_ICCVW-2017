...
local pathTrainData = '/media/aich/DATA/leaf_seg_par_dat/rgb';
local pathTrainLabels = '/media/aich/DATA/leaf_seg_par_dat/fg';

-- get mean estimate for of the training dataset
local meanData = torch.load("total_mean.dat")/MAX_IMG_VAL;
...
-- specify log file
trainLogger = optim.Logger('./logs/train_segnet_01_part1.log');
...
-- 				local jobDone = 0 -- this must be local
			...
			--				X[batch] = image.crop(im, 1, 1, 225, 225);
							...
							local tmpInputsCpu = torch.load(paths.concat(tmpPathTrainData, tostring(fileId) .. '.dat')):float();
							local tmpLabelsCpu = torch.load(paths.concat(tmpPathTrainLabels, tostring(fileId) .. '.dat')):char();
							-- starting indices for random cropping

							if (tmpInputsCpu:size(2) < options.inputSize) or (tmpInputsCpu:size(3) < options.inputSize) then
								local r_lb, r_ub, c_lb, c_ub, r_size, c_size;
								if tmpInputsCpu:size(2) < options.inputSize then
									r_lb = torch.floor((options.inputSize - tmpInputsCpu:size(2) )/2) + 1;
									r_ub = r_lb + tmpInputsCpu:size(2) - 1;
									r_size = options.inputSize;
								else
									r_lb = 1;
									r_ub = tmpInputsCpu:size(2);
									r_size = r_ub;
								end

								if tmpInputsCpu:size(3) < options.inputSize then
									c_lb = torch.floor((options.inputSize - tmpInputsCpu:size(3) )/2) + 1;
									c_ub = r_lb + tmpInputsCpu:size(3) - 1;
									c_size = options.inputSize;
								else
									c_lb = 1;
									c_ub = tmpInputsCpu:size(3);
									c_size = c_ub;
								end
--								print(tmpLabelsCpu:size());
--								print(tmpInputsCpu:size());
								local tmp1 = torch.zeros(3, r_size, c_size):type(torch.type(tmpInputsCpu)) ;
								local tmp2 = torch.zeros(r_size, c_size):type(torch.type(tmpLabelsCpu)) ;
								tmp1[{ {}, {r_lb, r_ub}, {c_lb, c_ub} }] = tmpInputsCpu;
								tmp2[{ {r_lb, r_ub}, {c_lb, c_ub} }] = tmpLabelsCpu;
								tmpInputsCpu = tmp1;
								tmpLabelsCpu = tmp2;
							end

							local ind1_lb = torch.ceil(torch.uniform(1e-2, tmpInputsCpu:size(2) - options.inputSize));
							local ind1_ub = ind1_lb + options.inputSize - 1;
							local ind2_lb = torch.ceil(torch.uniform(1e-2, tmpInputsCpu:size(3) - options.inputSize));
							local ind2_ub = ind2_lb + options.inputSize - 1;
							-- crop tensors
--							print(tmpInputsCpu:size(2), tmpInputsCpu:size(3), __threadid, ind1_lb, ind1_ub, ind2_lb, ind2_ub);
--							print(ind1_lb, ind1_ub, ind2_lb, ind2_ub);
--							print(ind1_ub - ind1_lb + 1, ind2_ub - ind2_lb + 1);
--							print('start ---' , tostring(__threadid));
							inputsCpu[placeId] = tmpInputsCpu[{ {}, {ind1_lb, ind1_ub}, {ind2_lb, ind2_ub} }];
							labelsCpu[placeId] = tmpLabelsCpu[{ {ind1_lb, ind1_ub}, {ind2_lb, ind2_ub} }];
--							print('end ---' , tostring(__threadid));
...
--				print(string.format('%d jobs done', jobDone));
		...
		precision_epoch = precision_epoch/tmpNumBatches;
		recall_epoch = recall_epoch/tmpNumBatches;
		accuracy_epoch = accuracy_epoch/tmpNumBatches;
		loss_epoch = loss_epoch/tmpNumBatches;
