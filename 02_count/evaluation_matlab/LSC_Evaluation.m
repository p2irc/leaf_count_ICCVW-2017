...
%   Date: 21.5.2014
%
% Copyright 2014, Forschungszentrum Juelich GmbH, 52425 Juelich, Germany
% 
%                          All Rights Reserved
% 
...
% Forschungszentrum Juelich GmbH not be used in advertising or publicity 
...
%
%
% THE AUTHOR AND FORSCHUNGSZENTRUM JUELICH GmbH DISCLAIM ALL WARRANTIES WITH 
% REGARD TO THIS SOFTWARE, INCLUDING ALL IMPLIED WARRANTIES OF 
% MERCHANTABILITY AND FITNESS FOR ANY PARTICULAR PURPOSE.  IN NO EVENT 
% SHALL THE AUTHOR OR FORSCHUNGSZENTRUM JUELICH GmbH BE LIABLE FOR ANY SPECIAL,  
...
% Juelich GmbH are obliged for bug fixes and other kinds of support.
...
%         are assumed to be named 'A1', 'A2', 'A3', 'A4' and/or 'A5'
...
%   - in the file path or file name a string 'A1', 'A2', 'A3', 'A4' or 'A5' needs to 
...
%   - for each experiment images are in a subfolder 'A1', 'A2', 'A3', 'A4' or 'A5'
...
%
% This function is used to evaluate results from the CVPPP 2017 Leaf
% Segmentation Challenge (see http://www.plant-phenotyping.org/CVPPP2017 for 
...
% inFolder
    ...
    filePattern = fullfile(inFolder, '*label.png')
    ...
    for k = 1:N
      ...
      elseif(~isempty(strfind(inFileName,'A4')) || ~isempty(strfind(inFileName,'a4')))
          experimentNumber(k)  = 4;
      elseif(~isempty(strfind(inFileName,'A5')) || ~isempty(strfind(inFileName,'a5')))
          experimentNumber(k)  = 5;
      else
...
function writeResultTable(resultName,saveFolder,username,imageNumberGT,bestDiceGT,fgbgDiceGT,absDiffLabelsGT,diffLabelsGT,experimentNumber)
    ...
    resultFileName = fullfile(saveFolder, [username '_' resultName '_results_LSC.csv']);
...
function CreateOverallFile(inpath,gtpath)
    ...
    num(4) = getNumberOfFiles(gtpath,'A4');
    num(5) = getNumberOfFiles(gtpath,'A5');
    overallNum = sum(num(:));
    % indices of result blocks per experiment
    idx = 1+[0,num(1),num(1)+num(2),num(1)+num(2)+num(3),num(1)+num(2)+num(3)+num(4),num(1)+num(2)+num(3)+num(4)+num(5)];
    ...
    for i=1:dirNumber
        for e=1:5 % loop experiments
            ...
            resBaseName = [dirNames{i} '_A' num2str(e,'%d') '_results_LSC.csv'];
...
function writeLaTeXTable(saveFolder,username,bestDiceGT,fgbgDiceGT,absDiffLabelsGT,diffLabelsGT,experimentNumber)
    ...
    % now just write the results to a csv table
    resultFileName = fullfile(saveFolder, [username '_results_LSC.tex']);
    fprintf(1, 'Writing results to %s\n', resultFileName);
