...
inCsvPath = '/media/aich/DATA/databases/leaf_cvppp2017/CVPPP2017_LCC_training/training';
inBasePath = '/media/aich/DATA/databases/leaf_cvppp2017/train_binSeg';
inRgbPath = 'rgb';
inFgPath = 'fg';
inBsPath = 'bs_sum_plain_nobox';
postImPath = {'A1','A2','A3','A4'};
outBasePath = '/media/aich/DATA/databases/leaf_cvppp2017/train_count';
outCsvPath = outBasePath;
outRgbPath = 'rgb';
outFgPath = 'fg';
outBsPath = 'bs';
...
for i = 1:length(postImPath)
    ...
    tmpOutFgPath = fullfile(outFgPath, postImPath{i});
    tmpOutBsPath = fullfile(outBsPath, postImPath{i});
    tmpInCsvPath = fullfile(inCsvPath, postImPath{i});
    tmpCsvFileName = [postImPath{i}, '.csv'];
    assert(copyfile(fullfile(tmpInCsvPath, tmpCsvFileName), outCsvPath), ...
            'Cannot copy CSV file = %s', fullfile(tmpInCsvPath, tmpCsvFileName));    
