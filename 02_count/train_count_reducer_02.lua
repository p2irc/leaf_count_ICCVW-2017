...
local outModelFile = "model_count_reducer_02";

os.execute("mkdir logs"); -- create log directory from terminal

--require './models/model_count_v1_pretrained.lua';
require './models/model_count_reducer_02.lua';
--require './models/model_count_reducer_pretrained_02.lua';
--require './models/replace_model_count_reducer_02.lua';
model:training();
-- require './models/model_count_v1.lua';

local numEpochs = 50; -- 35
local pathTrainData = '/media/aich/DATA/databases/lcc_seg_aug_dat';
local pathTrainLabels = '/media/aich/DATA/databases/leaf_cvppp2017/train_count';
local labelFileName = 'gt_seg_aug.mat';

-- specify log file
trainLogger = optim.Logger('./logs/train_count_reducer_02_part4.log');
...
local inputsCpu = torch.Tensor(1, 4, 448, 448);
local labelsCpu = torch.Tensor(1);
local inputs = torch.CudaTensor(1, 4, 448, 448);
...
--	optim.sgd(feval, parameters, optimState)
	optim.adam(feval, parameters, optimState) -- 1-19
	cutorch.synchronize();
